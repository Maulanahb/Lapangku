...
import 'package:lapangku/views/owner/widgets/owner_field_form_sheet.dart';
...
class _OwnerFieldsPageState extends ConsumerState<OwnerFieldsPage> {
  ...
  void _toggleStatus(String fieldId, bool currentStatus, String fieldName) async {
    try {
      await ref.read(ownerFieldProvider.notifier).toggleFieldStatus(fieldId, currentStatus);
      if (mounted) {
  ...
  void _showAddEditBottomSheet({OwnerFieldModel? field}) {
    showModalBottomSheet(
      ...
      builder: (context) {
        return OwnerFieldFormSheet(field: field);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    final fieldState = ref.watch(ownerFieldProvider);
