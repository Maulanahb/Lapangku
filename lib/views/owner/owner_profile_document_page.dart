...
class OwnerProfileDocumentPage extends ConsumerStatefulWidget {
  const OwnerProfileDocumentPage({super.key});

  @override
  ConsumerState<OwnerProfileDocumentPage> createState() =>
      _OwnerProfileDocumentPageState();
}

class _OwnerProfileDocumentPageState
    extends ConsumerState<OwnerProfileDocumentPage> {
  ...
  void initState() {
    super.initState();
    final profileState = ref.read(ownerProfileProvider).value;
    _businessNameController =
        TextEditingController(text: profileState?.businessName ?? '');
    _ownerNameController =
        TextEditingController(text: profileState?.ownerName ?? '');
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(ownerProfileProvider.notifier).updateProfile(
  ...
  Future<void> _pickAndUploadLogo() async {
    ...
    setState(() => _isLoading = true);
    try {
      await ref.read(ownerProfileProvider.notifier).uploadLogo(File(picked.path));
  ...
  Future<void> _pickAndUploadDoc(String docType) async {
    ...
    setState(() => _isLoading = true);
    try {
      await ref.read(ownerProfileProvider.notifier).uploadDocument(docType, File(picked.path));
