...
class OwnerPayoutPage extends ConsumerStatefulWidget {
  const OwnerPayoutPage({super.key});

  @override
  ConsumerState<OwnerPayoutPage> createState() => _OwnerPayoutPageState();
}

class _OwnerPayoutPageState extends ConsumerState<OwnerPayoutPage> {
  ...
  void initState() {
    super.initState();
    final state = ref.read(ownerProfileProvider).value;
    ...
    _accountNameController = TextEditingController(text: state?.ownerName ?? '');
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(ownerProfileProvider.notifier).updateBankInfo(
        _selectedBank ?? 'BCA',
