...
class _State extends ConsumerState<BookingConfirmationPage> {
  ...
  // Biaya layanan dari AppConstants — tidak hardcode di sini
  int get _biayaLayanan => AppConstants.serviceFee;
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              Expanded(
                  child: Text(widget.field.alamat,
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Harga lapangan', style: TextStyle(color: AppColors.textSecondary)),
          Text(CurrencyFormatter.format(_hargaLapangan), style: const TextStyle(fontWeight: FontWeight.w500)),
        ]),
        const SizedBox(height: 8),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Biaya layanan', style: TextStyle(color: AppColors.textSecondary)),
          Text(CurrencyFormatter.format(_biayaLayanan), style: const TextStyle(fontWeight: FontWeight.w500)),
        ]),
  ...
  Future<void> _handleBooking() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Silakan login terlebih dahulu'), backgroundColor: AppColors.error));
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() => _isBooking = false);
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal membuat pesanan: $e'), backgroundColor: AppColors.error));
      }
    }
  }
}
