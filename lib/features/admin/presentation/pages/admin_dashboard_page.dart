    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(
              icon: Icon(Icons.sports_soccer), label: 'Lapangan'),
          BottomNavigationBarItem(
...
class _DashboardBody extends ConsumerWidget {
  ...
  Widget _buildStatsGrid(AdminStats stats) {
    ...
    final cards = [
      ...
      _StatData('Pendapatan', 'Rp ${formatAngka(stats.totalPendapatan)}',
    ...
    return BarChart(
      BarChartData(
        ...
        barGroups: List.generate(data.length, (i) {
          final isMax =
              data[i] == data.reduce((a, b) => a > b ? a : b);
          return BarChartGroupData(
            x: i,
            barRods: [
              BarChartRodData(
                toY: data[i].toDouble(),
                color: isMax ? _primary : _primary.withOpacity(0.35),
                width: 28,
  ...
  Widget _buildDonutCard(bookings) {
    ...
    final berhasil =
        list.where((b) => b.status == 'selesai').length;
    final menunggu =
        list.where((b) => b.status == 'menunggu').length;
    final dibatalkan =
        list.where((b) => b.status == 'dibatalkan').length;
          ...
          Row(
            children: [
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _legendItem(_primary, 'Berhasil'),
                  const SizedBox(height: 8),
                  _legendItem(const Color(0xFFFFB74D), 'Menunggu'),
                  const SizedBox(height: 8),
  ...
  Widget _buildAktivitasItem(dynamic booking, bool isLast) {
    ...
    final inisial = nama.isNotEmpty
        ? nama.trim().split(' ').map((w) => w[0]).take(2).join()
        : '?';
