...
class _OwnerMainPageState extends State<OwnerMainPage> {
  ...
  // Tambahkan ValueKey agar AnimatedSwitcher tahu halamannya berbeda-beda
  final List<Widget> _pages = [
    const OwnerHomePage(key: ValueKey('home')),
    const ManageFieldsPage(key: ValueKey('fields')),
    const Center(
        key: ValueKey('orders'), child: Text('Halaman Orders (Belum Ada)')),
    const Center(
        key: ValueKey('profile'), child: Text('Halaman Profile (Belum Ada)')),
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // Kita ganti PageView dengan AnimatedSwitcher untuk efek Fade yang elegan
      body: AnimatedSwitcher(
        duration: const Duration(milliseconds: 250), // Kecepatan memudar
  ...
  Widget _buildNavItem(IconData icon, String label, int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        width: 85, // Lebar tetap yang tadi kita buat
